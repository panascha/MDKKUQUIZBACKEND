...
| `ai-gemini.gs` | Gemini key pool + `callGeminiAI`/`callGeminiConverter`. **AI_Config restructured 2026-07-19** to IntelSphere_Keys-style: `API_Key\|Donor_Name\|Status\|Last_Used\|Last_Reset_Date\|<model>_Remaining…` (per-model RPD, AI Studio free tier separates quota per model). Model registry lives in the **`AI_Models` sheet** (`Model\|RPD_Limit\|Priority\|Status\|Notes`) — add/disable models by editing that sheet, no redeploy. `getAvailableAIKey(provider, preferredModel?, reserveCount?)` picks (key, model) by registry priority + weighted-random across keys; `reserveCount > 0` holds back the top-N keys by total remaining for public users — only `agentQuery` (owner proxy) passes it (`AGENT_QUERY_GEMINI_KEY_RESERVE_COUNT = 1`), converter/student callers use the default 0 and can still draw every key; `updateAIUsage(apiKeyInfo, usedModel)` decrements the used model's column; converter 429 → `markModelExhausted_` zeroes that (key, model) for the day. GET actions: `setupAIConfig` (idempotent migrate/build), `aiConfigStatus` (masked diagnostic), `recoverAIConfigKeys&before=<ISO>` (restore keys from Drive revision history). Sheet writes wrapped in `aiSheetRetry_` — this spreadsheet intermittently throws "Spreadsheet service timed out" |
